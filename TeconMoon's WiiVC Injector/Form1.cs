...
namespace TeconMoon_s_WiiVC_Injector
{
    public partial class WiiVC_Injector : Form
    {
        ...
        private void TheBigOneTM_Click(object sender, EventArgs e)
        {
            ...
            string shortName = EscapeXml(PackedTitleLine1.Text);
            string longName = EnablePackedLine2.Checked && !string.IsNullOrWhiteSpace(PackedTitleLine2.Text)
            ...
            File.Copy(TempSourcePath + "TIKTEMP" + (File.Exists(TempSourcePath + "TIKTEMP\\tmd.bin") ? "" : "\\DATA") + "\\tmd.bin", TempBuildPath + "code\\rvlt.tmd");
            File.Copy(TempSourcePath + "TIKTEMP" + (File.Exists(TempSourcePath + "TIKTEMP\\ticket.bin") ? "" : "\\DATA") + "\\ticket.bin", TempBuildPath + "code\\rvlt.tik");
        ...
        private string EscapeXml(string inputString)
        {
            return System.Security.SecurityElement.Escape(inputString);
        }
    }
}
